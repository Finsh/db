...
BEGIN
	DELETE FROM `admin_do`.`answers` WHERE `id` = idAnswer AND `deleted` = 1;
END$$
	...
	IF (lenTEXT-constPart) > 0 THEN   
		...
		WHILE (@i <= countPartFullTEXT) DO
			...
			IF @i = 0 THEN
				...
				#SELECT MAX(`admin_do`.`questions`.`id`) INTO idRoot    
				#FROM `admin_do`.`questiosns`     
				#WHERE `admin_do`.`questions`.`extension` = NULL 
				#AND `admin_do`.`questions`.`partparagraphS_id1` = idPartparargraph; 
				COMMIT;
		...
		#SELECT * FROM `admin_do`.`questions`;   
...
BEGIN
	...
	UPDATE `admin_do`.`yodas` SET `classS_id` = idClass WHERE `id` =  idYoda;
	UPDATE `admin_do`.`classs` SET`yodaS_id` =  idYoda WHERE `id` = idClass;
...
BEGIN
	...
	DECLARE CURS CURSOR FOR SELECT id FROM `admin_do`.`subgroups` WHERE `isdefault`= true AND `classS_id1`=idClass;
	DECLARE CONTINUE HANDLER FOR SQLSTATE '02000' SET done=1;
	OPEN CURS;
	WHILE done = 0 DO
...
BEGIN
	INSERT INTO `admin_do`.`schedulerings`(`timestart`,`timefinsh`,`number`,`shift`,`schoolS_id`)
...
DELIMITER $$
USE `admin_do`;$$
CREATE PROCEDURE `admin_do`.`switchRightIncorrectAnswer` (
IN idAnswer INT UNSIGNED,
IN idQuestion INT UNSIGNED
)
BEGIN
